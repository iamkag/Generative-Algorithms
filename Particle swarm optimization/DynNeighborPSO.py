...
def get_Data(pso):
    ...
    newOrder = [shapely.affinity.rotate(
        shapely.affinity.translate(pso.Order[j], xoff=particle[j * 3], yoff=particle[j * 3 + 1]), particle[j * 3 + 2],
        origin='centroid') for j in range(len(pso.Order))]
    ...
    def __evaluateSwarm(self):
        ...
        if self.UseParallel:
            ...
            self.CurrentSwarmFitness[:] = Parallel(n_jobs=nCores)(
                delayed(self.ObjectiveFcn)(self.Swarm[i, :], self.nVars, self.Stock, self.Order) for i in
                range(nParticles))
        else:

            self.CurrentSwarmFitness[:] = [self.ObjectiveFcn(self.Swarm[i, :], self.nVars, self.Stock, self.Order) for i
                                           in range(nParticles)]
            ...
            for p in range(nParticles):
                ...
                self.Velocity[p, :] = self.AdaptiveInertia * self.Velocity[p, :] \
                                      ...
                                      + socialWeight * randSocial * (
                                                  self.PreviousBestPosition[bestNeighbor, :] - self.Swarm[p, :])
