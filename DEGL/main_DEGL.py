...
def run_DEGL():
    ...
    while (orders):
        ...
        for stockIdx in shapeIdx:
            ...
            degl = Degl(ObjectiveFcn, nVars, LowerBounds=LowerBounds, UpperBounds=UpperBounds,
                        OutputFcn=outFun, UseParallel=False, MaxStallIterations=15,a=0.9, b=0.9,
            ...
            deglalg = degl.GlobalBestPosition
            newOrder = [shapely.affinity.rotate(
                shapely.affinity.translate(currentOrder[k], xoff=deglalg[k * 3], yoff=deglalg[k * 3 + 1]),
                # ring pattern
                deglalg[k * 3 + 2], origin='centroid') for k in range(len(currentOrder))]
    ...
    fname = "results_DEGL/results_DEGL.csv"
    f = open(fname, "a+")
    ...
    name = "results_DEGL/result_DEGL.png"
