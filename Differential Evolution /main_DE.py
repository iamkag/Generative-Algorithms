...
def ObjectiveFcn(particle, nVars, Stock, Order):
    ...
    union = shapely.ops.unary_union(newOrder)  # the union of shapes with new positions and rotations
...
def get_Data(dealg, currentOrder, currentStock):
    newOrder = [shapely.affinity.rotate(
        shapely.affinity.translate(currentOrder[k], xoff=dealg[k * 3], yoff=dealg[k * 3 + 1]),
        dealg[k * 3 + 2], origin='centroid') for k in range(len(currentOrder))]

    unionNewOrder = shapely.ops.cascaded_union(newOrder)
    remaining = currentStock.difference(unionNewOrder)
...
class FigureObjects:
    ...
    def update(self, de, currentOrder, currentStock):
        ...
        xdata = np.arange(de.nit + 1) - 1
        self.lineBestFit.set_xdata(xdata)
        self.lineBestFit.set_ydata(de.fun)

        # update the global best fitness line (remember, -1 is for initialization == iteration 0)
        self.lineBestFit.set_ydata(de.fun)
        self.ax[0].relim()
        self.ax[0].autoscale_view()
        self.ax[0].title.set_text('Best-so-far global best fitness: {:g}'.format(de['fun']))
        ...
        newOrder, remaining = get_Data(de['x'], currentOrder, currentStock)
        ...
        self.ax[1].set_title('Rotated & translated order')
        self.ax[2].set_title('Remaining after set difference')
        pp = plotShapelyPoly(self.ax[1], [currentStock] + newOrder)
...
def run_DE(strategy):
    ...
    while (orders):
        ...
        for stockIdx in shapeIdx:
            ...
            obj_fun = lambda particle: ObjectiveFcn(particle, nVars, currentStock, currentOrder)
            ...
            de = differential_evolution(obj_fun, bounds, args=(), strategy=strategy, maxiter=50,
                                            tol=0.01,
                                            mutation=(0.5, 1.5), recombination=0.7, seed=None, callback=None,
                                            disp=False,
                                            polish=True, init='latinhypercube', atol=0.01, updating='immediate',
                                            constraints=(), x0=x0)

            figObj.update(de, currentOrder, currentStock)
            ...
            newOrder = [shapely.affinity.rotate(
                ...
                # ring pattern
                dealg[k * 3 + 2], origin='centroid') for k in range(len(currentOrder))]
            iterationsList.append(de['nit'])

            # first check if the order is in stock.
            union = shapely.ops.unary_union(newOrder)
            ...
            if (int(len(currentOrder)/2)) != 0:
                ...
                if not temp1:
                    if not temp2:
                        ...
                        orders = [temp2] + orders[1:]
                elif not temp2:
                    orders = [temp1] + orders[1:]
                else:
    ...
    fname = f"results_{strategy}/results_DE.csv"
    f = open(fname, "a+")
    ...
    name = f"results_{strategy}/result_DE.png"
...
if __name__ == '__main__':
    strategies = ['rand1bin', 'best1bin', 'best1exp', 'currenttobest1exp', 'best2bin']
