...
def run_nonDE_algorithms(algorithm):

    curr_method = algorithm
...
#            ros_sim = Simulator(ObjectiveFcn(x0,nVars,currentStock,currentOrder)
...
#                    res = minimize(ros_sim.simulate, [0, 0], method='BFGS', callback=ros_sim.callback, options={"disp": True})
                except:
...
#    MYDIR = [x.replace(' ', '') for x in MYDIR]
    ...
    fname = MYDIR + "/results" + curr_method + ".csv"
    f= open(fname,"a+")
...
if __name__ == '__main__':
    list_of_algorithms = ['Nelder-Mead', 'L-BFGS-B', 'SLSQP']

    for algorithm in list_of_algorithms:
        for i in range(5):
            run_nonDE_algorithms(algorithm)